 ...
 * 랜딩과 같은 재료로 만든다: 민트 카드, 3D 아이콘, 세리프 제목, 아이브로우. 에러가 아니라 "길 안내"이므로 붉은색을 쓰지 않는다.
 */
...
const [intro, action] = HQ_ONLY_NOTICE.lines;
</script>
...
<template>
  ...
  <div v-else class="hq hq-banner card card-mint" role="status">
    <span class="hq-ring hq-ring-sm" aria-hidden="true"><Icon3d name="team-hierarchy" :size="34" /></span>
    <div class="grow">
      <div class="hq-banner-title">{{ HQ_ONLY_NOTICE.title }}</div>
      <p class="hq-text hq-text-sm">{{ intro }} {{ action }}</p>
    </div>
...
<style scoped>
/* ── 공통 재료 ── */
.hq-ring { display: inline-grid; place-items: center; width: 96px; height: 96px; border-radius: 50%; background: var(--mint); box-shadow: inset 0 0 0 1px var(--brand-100); }
.hq-ring-sm { width: 56px; height: 56px; flex: none; }
...
.hq-art { display: flex; justify-content: center; padding: 36px 28px 0; }
.hq-body { padding: 20px 32px 8px; text-align: center; }
.hq-body .eyebrow { justify-content: center; }
.hq-title { font-size: 24px; margin: 8px 0 12px; color: var(--text); }
.hq-next { display: flex; align-items: center; gap: 12px; text-align: left; margin-top: 20px; padding: 14px 16px; background: var(--blush); border-radius: var(--radius-md); }
