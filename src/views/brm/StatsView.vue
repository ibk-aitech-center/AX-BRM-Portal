    ...
    <div class="dash">
      ...
      <Transition v-else name="swap" mode="out-in">
        <div v-if="stats" :key="dataKey" :aria-busy="loading" class="dash-body">
          ...
          <p class="hint">기준: 신청 일시(submitted_at) · 걸린 일수는 달력 기준으로 당일 처리를 1일로 셈(시각 차이는 세지 않음) · 첫 의견까지 평균은 접수 전 건 기준(의견 없이 종결된 건은 종결일까지, 아직 의견이 없는 진행 중 건은 오늘까지) · 요청부서는 신청 시점 소속 스냅샷 · 담당자는 현재 지정된 사람 · 목업 제공 = 컨셉 목업 첨부가 있는 건 · 작성 중(draft)은 제외 · 증감은 직전 같은 길이 기간과 비교</p>
        </div>
      </Transition>
    </div>
  </BrmShell>
