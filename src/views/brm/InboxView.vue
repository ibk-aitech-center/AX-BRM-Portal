...
async function load() {
  ...
  try {
    ...
    const [a, m] = await Promise.all([api.get<{ items: RequestSummary[]; statusCounts: Record<string, number> }>(`/api/requests?${p}`), api.get<InboxMeta>('/api/requests/meta')]);
    items.value = a.items; statusCounts.value = a.statusCounts || {}; orgs.value = m.orgs; assignees.value = m.assignees ?? []; unassigned.value = m.unassigned ?? 0;
